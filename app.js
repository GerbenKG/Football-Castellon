...
function finance(){
 ...
 const dueRows=[
   ...unpaidSeason.map(p=>'<tr><td>'+esc(p.name)+'</td><td>Season ticket</td><td>'+money(s.season_ticket_amount)+'</td><td>'+badge("Due","red")+'</td><td>'+(can("payments.manage")?'<label class="pay-check"><input class="check" type="checkbox" data-fin-due-ticket="'+p.id+'"><span>Paid</span></label>':"")+'</td></tr>'),
   ...unpaidGame.map(({g,x})=>'<tr><td>'+esc(x.guest?(x.name||"Guest"):(player(x.playerId)?.name||"Player"))+'</td><td>'+esc(dateText(g.date))+'</td><td>'+money(s.pay_per_game_amount)+'</td><td>'+badge("Due","red")+'</td><td>'+(can("payments.manage")?'<label class="pay-check"><input class="check" type="checkbox" data-fin-due-game="'+x.rowId+'"><span>Paid</span></label>':"")+'</td></tr>')
 ].join("");
...
console.info("[Football] APP BUILD 20260901-PAID-FIX loaded");
document.addEventListener("submit",async e=>{
 ...
 const isPlayerPickForm=e.target?.tagName==="FORM" && !!e.target.dataset.gameId && !!e.target.querySelector('select[name="id"]');
 if(isPlayerPickForm){
   ...
   if(g.participants.some(x=>!x.guest&&x.playerId===pid)){document.getElementById("modal-root").innerHTML="";render();return;}
   const payload={id:crypto.randomUUID(),game_id:selectedGameId,player_id:pid,guest_name:null,playing:true,attended:false,paid:false};
   const result=await sb.from("game_players").insert(payload).select("*").single();
   if(result.error){
     if(result.error.code==="23505"){await loadRemote({resetSelection:false});gameId=selectedGameId;document.getElementById("modal-root").innerHTML="";render();return;}
     alert("Could not add player to game: "+result.error.message);return;
   }
   ...
   gameId=selectedGameId;document.getElementById("modal-root").innerHTML="";render();return;
 }
 if(e.target.id==="guest-form"){
   ...
   g.participants.push(row);document.getElementById("modal-root").innerHTML="";render();return;
 }
