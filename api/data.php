...
const DATA_TABLES = [
    ...
    'game_players' => [
        'read' => 'attendance.view',
        'write' => 'attendance.manage',
...
{
    if (!isset(DATA_TABLES[$name])) {
        jsonResponse(['error' => 'Unsupported data table'], 400);
    }
...
function buildWhere(array $filters, array $allowedColumns, array &$params): string
{
    $parts = [];
    ...
    foreach ($filters as $index => $filter) {
        ...
        $param = ':f' . $index;
        $parts[] = sprintf('`%s` %s %s', $column, $operator === 'neq' ? '<>' : '=', $param);
...
if ($method === 'GET') {
    if (!hasPermission((string) $user['role'], $config['read'])) {
        jsonResponse(['error' => 'Permission denied'], 403);
    }

    $params = [];
    $sql = 'SELECT * FROM `' . $tableName . '`';
    $sql .= buildWhere(readFilters(), $config['columns'], $params);

    $order = (string) ($_GET['order'] ?? '');
    if ($order !== '') {
        [$orderColumn, $direction] = array_pad(explode(':', $order, 2), 2, 'asc');
        if (!in_array($orderColumn, $config['order'], true)) {
            jsonResponse(['error' => 'Invalid order column'], 400);
        }
        $sql .= ' ORDER BY `' . $orderColumn . '` ' . (strtolower($direction) === 'desc' ? 'DESC' : 'ASC');
    }
...
if (!hasPermission((string) $user['role'], $config['write'])) {
    jsonResponse(['error' => 'Permission denied'], 403);
}
...
if ($action === 'upsert' || $action === 'insert') {
    ...
    try {
        foreach ($rows as $row) {
            ...
            if (!isset($row['id'])) {
                throw new InvalidArgumentException('Every row must include id');
            }
...
if ($action === 'update') {
    if (!is_array($data) || !$data) {
        jsonResponse(['error' => 'Invalid update data'], 400);
    }
    if (!is_array($filters) || !$filters) {
        jsonResponse(['error' => 'Update requires a filter'], 400);
    }
    ...
    $sql .= buildWhere($filters, $config['columns'], $params);
...
if ($action === 'delete') {
    if (!is_array($filters) || !$filters) {
        jsonResponse(['error' => 'Delete requires a filter'], 400);
    }
