(() => {
  ...
  function selectedGame(data) {
    ...
    return data.games.find(g => dateKey(g.game_date) === heading.textContent.trim()) || null;
  }
  ...
  function applyDashboard(data) {
    ...
    const dueRows = payableRows.filter(r => {
      if (r.player_id && tickets.has(r.player_id)) return !tickets.get(r.player_id)?.paid;
      return !r.paid;
    });
  ...
  function applyFinance(data) {
    ...
    const payableGameRows = rows.filter(r => r.player_id || r.guest_name).filter(r => !tickets.has(r.player_id));
    ...
    const outstanding = unpaidSeason.reduce((a,p) => a + Number(tickets.get(p.id)?.amount || season.season_ticket_amount || 0), 0) + unpaidGame.length * Number(season.pay_per_game_amount || 0);
    ...
    if (dueTable) {
      ...
      if (tbody) {
        const rowsHtml = [
          ...unpaidSeason.map(p => `<tr><td>${esc(p.name)}</td><td>Season ticket</td><td>${money(tickets.get(p.id)?.amount || season.season_ticket_amount)}</td><td><span class="badge badge-red">Due</span></td></tr>`),
